class InterviewsController < ApplicationController
  ...
  def single_interview_submissions
     @submissions = @interview.submissions.where(current_no: 500, status: nil).paginate(:page => params[:page], :per_page => 5).order('created_at DESC')
  ...
  def filtered_single_interview
    @submissions = @interview.submissions.where(current_no: 500, status: params[:status].to_i).paginate(:page => params[:page], :per_page => 5).order('created_at DESC')
